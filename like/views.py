...
class Likes(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            try:
                comment = Comment.objects.get(pk=_request_params.get('comment'))
                try:
                    like = Like.objects.get(comment_id=comment.id, owner=request.user)
                    like.delete()
                    if _request_params.get('value') == like.value:
                        return JsonResponse(data={'msg': 'like or dislike deleted', 'success': True},
                                            status=HTTP_200_OK)
                    else:
                        raise Like.DoesNotExist
                except Like.DoesNotExist:
