 ...
 def post(self, request, identifier=None):
  ...
  if ser.is_valid():
   try:
    ...
    follow.delete()
    Notify.objects.create(**{'user': follow.chanel.owner, 'body': 'you are unfollowed by {}'.
           format(request.user.get_username()),
           "link": "{}/api/chanel/get/".format(request.get_host()) + str(
            follow.chanel.identifier)})
    ...
    Notify.objects.create(**{'user': follow.chanel.owner, 'body': 'you are followed by {}'.
                          format(request.user.get_username()),
